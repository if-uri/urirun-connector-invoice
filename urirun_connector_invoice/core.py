...
def _ksef_parse_xml_row(dirpath: str, fn: str, seen: set) -> tuple[dict, dict] | None:
    ...
    seen.add(key)
    row = {
        "file": fn, "number": f.get("number"), "issueDate": f.get("issueDate"),
...
def _ksef_update_totals(totals: dict, by_rate: dict, f: dict) -> None:
    for k in totals:
        if isinstance(f.get(k), (int, float)):
            totals[k] += f[k]
    ...
    for dirpath, _d, files in walker:
        for fn in sorted(files):
            if not fn.lower().endswith(".xml"):
                continue
            result = _ksef_parse_xml_row(dirpath, fn, seen)
            if result is None:
                continue
            row, f = result
            rows.append(row)
            _ksef_update_totals(totals, by_rate, f)
            if len(rows) >= max_files:
                break
...
def _receipt_body(text: str, receipt: dict[str, Any], path: str) -> str:
    return text or (str(receipt.get("text") or "")) or (_pdf_text(os.path.expanduser(path)) if path else "")


def _receipt_items_and_sum(receipt: dict[str, Any]) -> tuple[list, float | None]:
    ...
    return items, items_sum


def _build_receipt_draft(
    fields: dict[str, Any], receipt: dict[str, Any],
    items: list, items_sum: float | None,
    gross: float | None, net: float | None, vat: float | None,
    vat_rate: float, number: str, seller: str, buyer_nip: str, currency: str,
) -> dict[str, Any]:
    rate = float(vat_rate or 0)
    nip = _norm_nip(fields.get("nip") or receipt.get("nip"))
    return {
        "type": "invoice-draft", "source": "receipt",
        ...
        "vatRate": rate, "items": items, "itemsSum": items_sum,
        "net": net, "vat": vat, "gross": gross,
        "ksefReady": bool(nip and gross is not None),
    }


def _receipt_apply_llm(body: str, fields: dict[str, Any], model: str) -> None:
    llm = _llm_fields(body, model)
    if llm:
        for k, v in llm.items():
            if v not in (None, "") and not fields.get(k):
...
def _receipt_derive_amounts(
    fields: dict[str, Any], receipt: dict[str, Any], items_sum: float | None, vat_rate: float,
...
def _receipt_notes(gross: float | None, items_sum: float | None) -> list[str]:
    notes = []
    if gross is None:
...
def receipt_draft(text: str = "", receipt_json: str = "", path: str = "", vat_rate: float = 23.0,
    ...
    body = _receipt_body(text, receipt, path)
    ...
    if use_llm and body.strip() and not (fields.get("nip") and fields.get("gross")):
        _receipt_apply_llm(body, fields, model)

    items, items_sum = _receipt_items_and_sum(receipt)
    gross, net, vat = _receipt_derive_amounts(fields, receipt, items_sum, vat_rate)
    draft = _build_receipt_draft(fields, receipt, items, items_sum, gross, net, vat,
                                 vat_rate, number, seller, buyer_nip, currency)
    return {"ok": gross is not None, "connector": CONNECTOR_ID, "kind": "invoice-draft", "live": False,
            "draft": draft, "fields": fields, "notes": _receipt_notes(gross, items_sum)}
...
def _fa2_sub(parent, tag, text=None, **attrs):
...
def _fa2_naglowek(root, created_at: str, system_info: str) -> None:
    nag = _fa2_sub(root, "Naglowek")
    _fa2_sub(nag, "KodFormularza", "FA", kodSystemowy="FA (2)", wersjaSchemy="1-0E")
    _fa2_sub(nag, "WariantFormularza", "2")
    _fa2_sub(nag, "DataWytworzeniaFa", created_at or datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"))
    _fa2_sub(nag, "SystemInfo", system_info)


def _fa2_podmiot_block(root, tag: str, nip: str, name: str, address: str) -> None:
    p = _fa2_sub(root, tag)
    did = _fa2_sub(p, "DaneIdentyfikacyjne")
    _fa2_sub(did, "NIP", nip)
    _fa2_sub(did, "Nazwa", name)
    adr = _fa2_sub(p, "Adres")
    _fa2_sub(adr, "KodKraju", "PL")
    _fa2_sub(adr, "AdresL1", address)


def _fa2_adnotacje(fa) -> None:
    adn = _fa2_sub(fa, "Adnotacje")
    for tag, val in [("P_16", "2"), ("P_17", "2"), ("P_18", "2"), ("P_18A", "2")]:
        _fa2_sub(adn, tag, val)
    _fa2_sub(_fa2_sub(adn, "Zwolnienie"), "P_19N", "1")
    _fa2_sub(_fa2_sub(adn, "NoweSrodkiTransportu"), "P_22N", "1")
    _fa2_sub(adn, "P_23", "2")
    _fa2_sub(_fa2_sub(adn, "PMarzy"), "P_PMarzyN", "1")


def _fa2_amounts(fa, draft: dict[str, Any], idx: str) -> None:
    if draft.get("net") is not None:
        _fa2_sub(fa, f"P_13_{idx}", _q2(draft["net"]))
    if draft.get("vat") is not None:
        _fa2_sub(fa, f"P_14_{idx}", _q2(draft["vat"]))
    if draft.get("gross") is not None:
        _fa2_sub(fa, "P_15", _q2(draft["gross"]))


def _fa2_fa_wiersz(fa, i: int, item: dict[str, Any], rate: int) -> None:
    gross_line = float(item.get("price") or 0)
    net_line = round(gross_line / (1 + rate / 100.0), 2) if rate else gross_line
    w = _fa2_sub(fa, "FaWiersz")
    _fa2_sub(w, "NrWierszaFa", str(i))
    _fa2_sub(w, "P_7", str(item.get("name") or f"Pozycja {i}"))
    _fa2_sub(w, "P_8B", "1")
    _fa2_sub(w, "P_9A", _q2(net_line))
    _fa2_sub(w, "P_11", _q2(net_line))
    _fa2_sub(w, "P_12", str(rate))
...
def _build_fa2_xml(draft: dict[str, Any], *, system_info: str = "ifURI",
    ...
    _fa2_naglowek(root, created_at, system_info)
    _fa2_podmiot_block(root, "Podmiot1",
                       draft.get("sellerNip") or "0000000000",
                       draft.get("seller") or "Sprzedawca",
                       seller_address or "ul. Przykładowa 1, 00-001 Miasto")
    _fa2_podmiot_block(root, "Podmiot2",
                       draft.get("buyerNip") or "9999999999",
                       draft.get("buyer") or "Nabywca",
                       buyer_address or "ul. Nabywcza 2, 00-002 Miasto")
    fa = _fa2_sub(root, "Fa")
    _fa2_sub(fa, "KodWaluty", draft.get("currency") or "PLN")
    _fa2_sub(fa, "P_1", draft.get("issueDate") or datetime.now(timezone.utc).strftime("%Y-%m-%d"))
    _fa2_sub(fa, "P_2", number or draft.get("number") or "DRAFT")
    _fa2_sub(fa, "P_6", sale_date or draft.get("issueDate") or "")
    rate = int(round(float(draft.get("vatRate") or 23)))
    _fa2_amounts(fa, draft, _RATE_INDEX.get(rate, "1"))
    _fa2_adnotacje(fa)
    _fa2_sub(fa, "RodzajFaktury", "VAT")
    for i, item in enumerate(draft.get("items") or [], start=1):
        _fa2_fa_wiersz(fa, i, item, rate)
...
def _sv_required_elements(root, fa) -> list[str]:
    errors: list[str] = []
...
def _sv_parties(root) -> tuple[list[str], list[str]]:
    errors: list[str] = []
    warnings: list[str] = []
...
def _sv_arithmetic(fa) -> list[str]:
    if fa is None:
        return []
    net = vat = None
    for el in fa.iter():
    ...
    if net is not None and vat is not None and gross is not None and abs(round(net + vat, 2) - gross) > 0.02:
        return [f"net+VAT ({round(net + vat, 2)}) != P_15 gross ({gross})"]
    return []


def _sv_nip_format(root) -> list[str]:
    p1 = _sub(root, "Podmiot1")
    nip = _txt(p1, "NIP") if p1 is not None else None
    if nip and len(re.sub(r'\D', '', nip)) != 10:
        return [f"seller NIP '{nip}' is not 10 digits"]
    return []
...
def _structural_validate(data) -> tuple[list[str], list[str]]:
    ...
    fa = _sub(root, "Fa")
    errors = _sv_required_elements(root, fa)
    p_errs, p_warns = _sv_parties(root)
    errors += p_errs
    warnings += p_warns
    errors += _sv_arithmetic(fa)
    warnings += _sv_nip_format(root)
    return errors, warnings
